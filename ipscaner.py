...
hint="""
  ...
  |__________________________|
    """       
print(f'{Fore.CYAN+logo}{Fore.GREEN+hint}')    
...
def gethostname(address, q, hostnames, nb):
    ...
    if (host_mac != None):
        ...
        if (res != None):
            ...
            host_port = None
            #ports = range (0,65535)
            ports = (80,443,22,21,25,53,139,3389)
            allports = ""
            for port in ports:
...
if (arg_json == 0):
    print (f"\n{Fore.RED}Scanning : {Fore.LIGHTYELLOW_EX + cidr}.0{Fore.YELLOW}/24 \n{Fore.RED}Your gateway : {Fore.LIGHTMAGENTA_EX + gateway}")
total = 0
q = queue.Queue()
threads = []
hostnames = {}
...
for t in threads:
    t.start()
if (arg_json == 0):
    x= Fore.LIGHTRED_EX+"SCANNING Level " + Fore.LIGHTGREEN_EX
    with FillingSquaresBar(x, max=254) as bar:        
        for t in threads:
            t.join()
            bar.next()
else:        
